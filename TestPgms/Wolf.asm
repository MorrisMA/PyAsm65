...
;  115:     FOR i := 0 TO max DO BEGIN
	lda #0	;---	mov		ax,0
	sta.w i_014,X	;---	mov		WORD PTR i_014,ax
L_017
	lda #10	;---	mov		ax,10
	cmp.w i_014,X	;---	cmp		WORD PTR i_014,ax
	bge L_018	;---	jle		L_018
	jmp L_019	;---	jmp		L_019
L_018
;  116:         island[0,   i] := border;
	...
	lda.w i_014,X	;---	mov		ax,WORD PTR i_014
						;---	mov		dx,2
						;---	imul	dx
...
;  119:         island[i, max] := border;
	...
	adj #2	;--- pop TOS
;  120:     END;
	inc.w i_014,X	;---	inc		WORD PTR i_014
	jmp L_017	;---	jmp		L_017
L_019
	dec.w i_014,X	;---	dec		WORD PTR i_014
;  121:     FOR row := 1 TO size DO BEGIN
	lda #1	;---	mov		ax,1
	sta.w row_015,X	;---	mov		WORD PTR row_015,ax
L_020
	lda #9	;---	mov		ax,9
	cmp.w row_015,X	;---	cmp		WORD PTR row_015,ax
	bge L_021	;---	jle		L_021
	jmp L_022	;---	jmp		L_022
L_021
;  122:         FOR col := 1 TO size DO BEGIN
	lda #1	;---	mov		ax,1
	sta.w col_016,X	;---	mov		WORD PTR col_016,ax
L_023
	lda #9	;---	mov		ax,9
	cmp.w col_016,X	;---	cmp		WORD PTR col_016,ax
	bge L_024	;---	jle		L_024
	jmp L_025	;---	jmp		L_025
L_024
;  123:         island[row, col]    := empty;
	psh.w #island_002	;---	lea		ax,WORD PTR island_002
	lda.w row_015,X	;---	mov		ax,WORD PTR row_015
						;---	mov		dx,22
						;---	imul	dx
	pha.w	;push index
	psh.w #22	;push element size
	...
	adc.w 0,S	;add index offset to array base
	sta.w 0,S	;store address of array element ---	push	dx
	lda.w col_016,X	;---	mov		ax,WORD PTR col_016
						;---	mov		dx,2
						;---	imul	dx
	asl.w a	;arithmetic shift
						;---	pop		dx
...
;  124:         foodunits[row, col] := 0;
	...
	adj #2	;--- pop TOS
;  125:         END;
	inc.w col_016,X	;---	inc		WORD PTR col_016
	jmp L_023	;---	jmp		L_023
L_025
	dec.w col_016,X	;---	dec		WORD PTR col_016
;  126:     END;
	inc.w row_015,X	;---	inc		WORD PTR row_015
	jmp L_020	;---	jmp		L_020
L_022
	dec.w row_015,X	;---	dec		WORD PTR row_015
;  127: 
...
;  169:     random := (seed*limit) DIV divisor;
	lda.w seed_010	;---	mov		ax,WORD PTR seed_010
	pha.w	;---	push	ax
	lda.w limit_036,X	;---	mov		ax,WORD PTR limit_036
	pha.w	;---	pop		dx
	jsr _imul	;---	imul	dx
	adj #4	;pop ops/params
	pha.w	;---	push	ax
	...
	jsr _idiv	;---	idiv	cx
	adj #4	;pop ops/params
	sta.w RETURN_VALUE,X	;---	mov		RETURN_VALUE,ax
;  170:     END {random};
	lda.w RETURN_VALUE,X	;---	mov		ax,RETURN_VALUE
	txs.w	;---	mov		sp,bp
	plx.w	;---	pop		bp
	rts	;---	ret		4
	.ENDP	random_035
...
;  192:         adj := 0;
	lda #0	;---	mov		ax,0
	sta.w adj_043,X	;---	mov		WORD PTR adj_043,ax
;  193:         REPEAT
L_049
;  194:         adj := adj + 1;
	lda.w adj_043,X	;---	mov		ax,WORD PTR adj_043
	pha.w	;---	push	ax
	lda #1	;---	mov		ax,1
						;---	pop		dx
	clc	;---	add		ax,dx
...
;  195:         newrow := oldrow + rowoffset[adj];
	lda.w newrow_041,X	;---	mov		ax,WORD PTR newrow_041
	pha.w	;---	push	ax
	lda.w oldrow_039,X	;---	mov		ax,WORD PTR oldrow_039
	pha.w	;---	push	ax
	psh.w #rowoffset_011	;---	lea		ax,WORD PTR rowoffset_011
	lda.w adj_043,X	;---	mov		ax,WORD PTR adj_043
						;---	mov		dx,2
...
;  196:         newcol := oldcol + coloffset[adj];
	lda.w newcol_042,X	;---	mov		ax,WORD PTR newcol_042
	pha.w	;---	push	ax
	lda.w oldcol_040,X	;---	mov		ax,WORD PTR oldcol_040
	pha.w	;---	push	ax
	psh.w #coloffset_012	;---	lea		ax,WORD PTR coloffset_012
	lda.w adj_043,X	;---	mov		ax,WORD PTR adj_043
						;---	mov		dx,2
...
;  197:         what   := island[newrow, newcol];
	...
	lda.w (newrow_041,X)	;---	mov		ax,WORD PTR [bx]
						;---	mov		dx,22
						;---	imul	dx
	pha.w	;push index
	psh.w #22	;push element size
	...
	lda.w (newcol_042,X)	;---	mov		ax,WORD PTR [bx]
						;---	mov		dx,2
						;---	imul	dx
	asl.w a	;arithmetic shift
						;---	pop		dx
	...
	lda.w (0,S)	;---	mov		ax,WORD PTR [bx]
	adj #2	;pop ops/params
	sta.w what_044,X	;---	mov		WORD PTR what_044,ax
;  198:         done   := what = rabbit;
	lda.w what_044,X	;---	mov		ax,WORD PTR what_044
	pha.w	;---	push	ax
	lda #1	;---	mov		ax,1
	xma.w 0,S	;---	pop		dx
	cmp.w 0,S	;---	cmp		dx,ax
...
;  199:         UNTIL done OR (adj = 4);
	lda.w done_045,X	;---	mov		ax,WORD PTR done_045
	pha.w	;---	push	ax
	lda.w adj_043,X	;---	mov		ax,WORD PTR adj_043
	pha.w	;---	push	ax
	lda #4	;---	mov		ax,4
...
;  205:         adj := random(5);
	lda #5	;---	mov		ax,5
	pha.w	;---	push	ax
	lda.w STATIC_LINK,X	;---	push	STATIC_LINK
	pha.w	;push acc
...
;  226:     foodunits[oldrow, oldcol] := foodunits[oldrow, oldcol] - 1;
	psh.w #foodunits_003	;---	lea		ax,WORD PTR foodunits_003
	lda.w oldrow_061,X	;---	mov		ax,WORD PTR oldrow_061
	dec.w a	;---	sub		ax,1
						;---	mov		dx,18
						;---	imul	dx
	pha.w	;push index
	...
	adc.w 0,S	;add index offset to array base
	sta.w 0,S	;store address of array element ---	push	dx
	lda.w oldcol_062,X	;---	mov		ax,WORD PTR oldcol_062
	dec.w a	;---	sub		ax,1
						;---	mov		dx,2
						;---	imul	dx
	asl.w a	;arithmetic shift
...
;  240:         moved := (newrow <> oldrow) OR (newcol <> oldcol);
	lda.w newrow_063,X	;---	mov		ax,WORD PTR newrow_063
	pha.w	;---	push	ax
	lda.w oldrow_061,X	;---	mov		ax,WORD PTR oldrow_061
	xma.w 0,S	;---	pop		dx
	cmp.w 0,S	;---	cmp		dx,ax
...
L_073
	pha.w	;---	push	ax
	lda.w newcol_064,X	;---	mov		ax,WORD PTR newcol_064
	pha.w	;---	push	ax
	lda.w oldcol_062,X	;---	mov		ax,WORD PTR oldcol_062
	xma.w 0,S	;---	pop		dx
	cmp.w 0,S	;---	cmp		dx,ax
...
L_074
	...
	ora.w 0,S	;---	or		ax,dx
	adj #2	;pop ops/params
	sta.w moved_065,X	;---	mov		WORD PTR moved_065,ax
;  241: 
;  242:         IF moved THEN BEGIN
	lda.w moved_065,X	;---	mov		ax,WORD PTR moved_065
	cmp.w #1	;---	cmp		ax,1
	beq L_075	;---	je		L_075
	jmp  L_076	;---	jmp		L_076
L_075
...
;  291:     NewLocation(rabbit, oldrow, oldcol, newrow, newcol);
	lda #1	;---	mov		ax,1
	pha.w	;---	push	ax
	lda.w oldrow_089,X	;---	mov		ax,WORD PTR oldrow_089
	pha.w	;---	push	ax
	lda.w oldcol_090,X	;---	mov		ax,WORD PTR oldcol_090
	pha.w	;---	push	ax
...
;  292:     moved := (newrow <> oldrow) OR (newcol <> oldcol);
	lda.w newrow_091,X	;---	mov		ax,WORD PTR newrow_091
	pha.w	;---	push	ax
	lda.w oldrow_089,X	;---	mov		ax,WORD PTR oldrow_089
	xma.w 0,S	;---	pop		dx
	cmp.w 0,S	;---	cmp		dx,ax
...
L_094
	pha.w	;---	push	ax
	lda.w newcol_092,X	;---	mov		ax,WORD PTR newcol_092
	pha.w	;---	push	ax
	lda.w oldcol_090,X	;---	mov		ax,WORD PTR oldcol_090
	xma.w 0,S	;---	pop		dx
	cmp.w 0,S	;---	cmp		dx,ax
...
L_095
	...
	ora.w 0,S	;---	or		ax,dx
	adj #2	;pop ops/params
	sta.w moved_093,X	;---	mov		WORD PTR moved_093,ax
;  293:     IF moved THEN BEGIN
	lda.w moved_093,X	;---	mov		ax,WORD PTR moved_093
	cmp.w #1	;---	cmp		ax,1
	beq L_096	;---	je		L_096
	jmp  L_097	;---	jmp		L_097
L_096
...
;  322:     FOR row := 1 TO size DO BEGIN
	lda #1	;---	mov		ax,1
	sta.w row_105,X	;---	mov		WORD PTR row_105,ax
L_107
	lda #9	;---	mov		ax,9
	cmp.w row_105,X	;---	cmp		WORD PTR row_105,ax
	bge L_108	;---	jle		L_108
	jmp L_109	;---	jmp		L_109
L_108
;  323:         FOR col := 1 TO size DO BEGIN
	lda #1	;---	mov		ax,1
	sta.w col_106,X	;---	mov		WORD PTR col_106,ax
L_110
	lda #9	;---	mov		ax,9
	cmp.w col_106,X	;---	cmp		WORD PTR col_106,ax
	bge L_111	;---	jle		L_111
	jmp L_112	;---	jmp		L_112
L_111
;  324:         IF island[row, col] = wolf THEN BEGIN
	psh.w #island_002	;---	lea		ax,WORD PTR island_002
	lda.w row_105,X	;---	mov		ax,WORD PTR row_105
						;---	mov		dx,22
						;---	imul	dx
	pha.w	;push index
	psh.w #22	;push element size
	...
	adc.w 0,S	;add index offset to array base
	sta.w 0,S	;store address of array element ---	push	dx
	lda.w col_106,X	;---	mov		ax,WORD PTR col_106
						;---	mov		dx,2
						;---	imul	dx
	asl.w a	;arithmetic shift
						;---	pop		dx
...
L_114
;  327:         END;
	inc.w col_106,X	;---	inc		WORD PTR col_106
	jmp L_110	;---	jmp		L_110
L_112
	dec.w col_106,X	;---	dec		WORD PTR col_106
;  328:     END;
	inc.w row_105,X	;---	inc		WORD PTR row_105
	jmp L_107	;---	jmp		L_107
L_109
	dec.w row_105,X	;---	dec		WORD PTR row_105
;  329: 
;  330: 
...
;  355:     FOR row := 1 TO size DO BEGIN
	lda #1	;---	mov		ax,1
	sta.w row_126,X	;---	mov		WORD PTR row_126,ax
L_130
	lda #9	;---	mov		ax,9
	cmp.w row_126,X	;---	cmp		WORD PTR row_126,ax
	bge L_131	;---	jle		L_131
	jmp L_132	;---	jmp		L_132
L_131
;  356:         write(' ':10);
...
;  357:         FOR col := 1 TO size DO BEGIN
	lda #1	;---	mov		ax,1
	sta.w col_127,X	;---	mov		WORD PTR col_127,ax
L_133
	lda #9	;---	mov		ax,9
	cmp.w col_127,X	;---	cmp		WORD PTR col_127,ax
	bge L_134	;---	jle		L_134
	jmp L_135	;---	jmp		L_135
L_134
;  358:         cnts := island[row, col];
	psh.w #island_002	;---	lea		ax,WORD PTR island_002
	lda.w row_126,X	;---	mov		ax,WORD PTR row_126
						;---	mov		dx,22
						;---	imul	dx
	pha.w	;push index
	psh.w #22	;push element size
	...
	adc.w 0,S	;add index offset to array base
	sta.w 0,S	;store address of array element ---	push	dx
	lda.w col_127,X	;---	mov		ax,WORD PTR col_127
						;---	mov		dx,2
						;---	imul	dx
	asl.w a	;arithmetic shift
						;---	pop		dx
	...
	lda.w (0,S)	;---	mov		ax,WORD PTR [bx]
	adj #2	;pop ops/params
	sta.w cnts_128,X	;---	mov		WORD PTR cnts_128,ax
;  359:         IF      cnts = empty  THEN write('. ')
	lda.w cnts_128,X	;---	mov		ax,WORD PTR cnts_128
	pha.w	;---	push	ax
	lda #4	;---	mov		ax,4
	xma.w 0,S	;---	pop		dx
	cmp.w 0,S	;---	cmp		dx,ax
...
L_145
L_140
	inc.w col_127,X	;---	inc		WORD PTR col_127
	jmp L_133	;---	jmp		L_133
L_135
	dec.w col_127,X	;---	dec		WORD PTR col_127
;  363:         writeln;
	jsr _writeln	;---	call	_write_line
;  364:     END;
	inc.w row_126,X	;---	inc		WORD PTR row_126
	jmp L_130	;---	jmp		L_130
L_132
	dec.w row_126,X	;---	dec		WORD PTR row_126
;  365:     END {PrintIsland};
	txs.w	;---	mov		sp,bp
	plx.w	;---	pop		bp
	rts	;---	ret		2
...
;  377:     FOR row := 1 TO size DO BEGIN
	lda #1	;---	mov		ax,1
	sta.w row_151,X	;---	mov		WORD PTR row_151,ax
L_153
	lda #9	;---	mov		ax,9
	cmp.w row_151,X	;---	cmp		WORD PTR row_151,ax
	bge L_154	;---	jle		L_154
	jmp L_155	;---	jmp		L_155
L_154
;  378:         FOR col := 1 TO size DO BEGIN
	lda #1	;---	mov		ax,1
	sta.w col_152,X	;---	mov		WORD PTR col_152,ax
L_156
	lda #9	;---	mov		ax,9
	cmp.w col_152,X	;---	cmp		WORD PTR col_152,ax
	bge L_157	;---	jle		L_157
	jmp L_158	;---	jmp		L_158
L_157
;  379:         IF island[row, col] = newwolf THEN BEGIN
	psh.w #island_002	;---	lea		ax,WORD PTR island_002
	lda.w row_151,X	;---	mov		ax,WORD PTR row_151
						;---	mov		dx,22
						;---	imul	dx
	pha.w	;push index
	psh.w #22	;push element size
	...
	adc.w 0,S	;add index offset to array base
	sta.w 0,S	;store address of array element ---	push	dx
	lda.w col_152,X	;---	mov		ax,WORD PTR col_152
						;---	mov		dx,2
						;---	imul	dx
	asl.w a	;arithmetic shift
						;---	pop		dx
...
L_162
;  385:         END;
	inc.w col_152,X	;---	inc		WORD PTR col_152
	jmp L_156	;---	jmp		L_156
L_158
	dec.w col_152,X	;---	dec		WORD PTR col_152
;  386:     END;
	inc.w row_151,X	;---	inc		WORD PTR row_151
	jmp L_153	;---	jmp		L_153
L_155
	dec.w row_151,X	;---	dec		WORD PTR row_151
;  387:     END {ResetIsland};
	txs.w	;---	mov		sp,bp
	plx.w	;---	pop		bp
	rts	;---	ret		2
